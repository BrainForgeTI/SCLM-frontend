...
const MyAdventurePage = () => {
  ...
  const [adventureBeforeEdit, setAdventureBeforeEdit] =
    useState<AdventureCardType | null>(null);

  const [deletedTopicsList, setDeletedTopicsList] = useState<
    DeletedChapterTopic[]
  >([]);
  const [deletedChaptersList, setDeletedChaptersList] = useState<
    DeletedChapterType[]
  >([]);
  const [modifiedTopicsList, setModifiedTopicsList] = useState<
    ModifyChapterTopics[]
  >([]);
  const [modifiedTitleList, setModifiedTitleList] = useState<
    ModifyChapterTitle[]
  >([]);
  ...
  async function handleChapterTopicCompleted(
    chapterId: string,
    topicId: string,
    completed: boolean,
  ) {
  ...
  async function addChapterToAdventure(chapter: ChapterType) {
    if (!validator.emptyString(chapter.title)) {
      if (adventureContext.adventure && adventureContext.adventure.id) {
        ...
        let response = await api.createChapter({
          adventureId: adventureContext.adventure.id,
          chapterTitle: chapter.title,
        });
        if (response.status !== 201) {
          adventure.chapters = adventure.chapters.filter(
            (current) => current.id !== chapter.id,
          );
          adventureContext.setAdventure(adventure);
          setNewChapter(chapter);
        } else {
          adventure.chapters = adventure.chapters.map((current) =>
            current.id === chapter.id
              ? { ...current, id: response.chapterId! }
              : current,
          );
          adventureContext.setAdventure(adventure);
    ...
    if (!validator.emptyString(topic.name)) {
      if (adventureContext.adventure && adventureContext.adventure.id) {
        ...
        adventure.chapters.map((chapter) =>
          chapter.id == chapterId ? chapter.topics.push(topic) : chapter,
        );

        adventureContext.setAdventure(adventure);
        setNewTopic(null);

        let response = await api.createChapterTopic({
          adventureId: adventureContext.adventure.id,
          chapterId: chapterId,
          topic: topic,
        });
      ...
      const filteredModifiedTopicWithDeletedTopics = modifiedTopicsList.filter(
        (modifiedTopic) =>
          !filteredDeletedTopics.some(
            (deleteItem) =>
              modifiedTopic.chapterId === deleteItem.chapterId &&
              modifiedTopic.topic.id === deleteItem.topicId,
          ),
      );

      const filteredModifiedArray =
        filteredModifiedTopicWithDeletedTopics.filter(
          (modifiedTopic) =>
            !deletedChaptersList.some(
              (deletedChapter) =>
  ...
  function removeFromModifiedTitleList(chapterId: string) {
    let newArray = modifiedTitleList.filter(
      (title) => title.chapterId !== chapterId,
    );
    setModifiedTitleList(newArray);
  }

  function putInDeletedList(chapterId: string, topicId: string) {
    setDeletedTopicsList((prev) => [
      ...prev,
      { chapterId: chapterId, topicId: topicId },
    ]);
  }
  ...
  function removeFromDeletedList(chapterId: string, topicId: string) {
    let newArray = deletedTopicsList.filter(
      (deletedTopic) =>
        !(
          deletedTopic.chapterId === chapterId &&
          deletedTopic.topicId === topicId
        ),
    );
  ...
  function putInModifiedTopicsList(chapterId: string, topic: TopicType) {
    ...
    if (!updated) {
      setModifiedTopicsList((prev) => [
        ...prev,
        { chapterId: chapterId, topic: topic },
      ]);
    }
  }

  function removeFromModifiedList(chapterId: string, topicId: string) {
    let newArray = modifiedTopicsList.filter(
      (topicArr) =>
        !(topicArr.topic.id === topicId && topicArr.chapterId === chapterId),
    );
              ...
              adventureContext.adventure.chapters ? (
                <div className="min-w-[100px] md:min-w-[137px]">
                  <ActionButton
                    ...
                    label={`${editMode ? "Salvar" : "Editar"}`}
                    style={`border gap-5 text-action-prev-content ${editMode ? "bg-action-prev border-neutral/38" : "bg-action-prev/11 border-action-prev/89"}`}
                  />
                </div>
              ) : (
                <></>
              ...
              {editMode && (
                <div className="min-w-[100px] md:min-w-[137px]">
                  <ActionButton
                    action={restoreEdits}
                    label="Cancelar"
                    style="bg-error/50 border border-error text-error-content"
                  />
                </div>
              )}
            </div>
          ...
          <>
            ...
            adventureContext.adventure.chapters.length > 0 ? (
              adventureContext.adventure.chapters.map((chapter, index) => {
                ...
                if (canShow) {
                  let deleted = deletedChaptersList.some(
                    (current) => current.chapterId === chapter.id,
                  );
                  return (
                    <div className="flex flex-col">
                      ...
                      <ChapterSection
                        addTopicToAdventure={addTopicToAdventure}
                        newTopic={newTopic}
                        setNewTopic={setNewTopic}
                        putInModifiedTitleList={putInModifiedTitleList}
                        removeFromModifiedTitleList={
                          removeFromModifiedTitleList
                        }
                        deletedTopicsList={deletedTopicsList}
                        modifiedTopicsList={modifiedTopicsList}
                        putInDeletedList={putInDeletedList}
                        putInModifiedTopicsList={putInModifiedTopicsList}
                        removeFromDeletedList={removeFromDeletedList}
                        removeFromModifiedList={removeFromModifiedList}
                        key={`chapter-${index}`}
                        editMode={editMode}
            ...
            {newChapter !== null ? (
              <div className="w-full flex flex-col">
                ...
                <ChapterSection
                  ...
                  index={`${(adventureContext.adventure?.chapters.length || 0) + 1}`}
                />
              </div>
            ) : (
              <></>
          ...
          newChapter === null ? (
            <div className="w-full mb-10">
              <ActionButton
                ...
                label="+ Novo Capítulo"
                style="bg-secondary/10 text-secondary-content font-semibold py-5 border border-neutral/17 hover:border-neutral/50 hover:scale-[1.001]"
