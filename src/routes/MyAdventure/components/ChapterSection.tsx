...
const ChapterSection = (props: Props) => {
  ...
  const allCompleted =
    ...
    props.chapter.topics.every((topic) => topic.completed);
  ...
  return (
    <div className="w-full relative min-h-[63px] overflow-hidden border rounded-lg">
      <div
        onClick={() => {
          props.handleExpand(props.chapter.id);
        }}
        ...
        {props.chapter.id !== "new-chapter" ? (
          <>
            {props.chapter.expanded ? (
              <button className="h-full cursor-pointer shrink-0 px-2 pe-5">
                <ArrowUpIcon />
              </button>
            ) : (
              <button className="h-full cursor-pointer shrink-0 px-2 pe-5">
      ...
      <div
        ...
        className={`px-5 lg:ps-20 w-full mt-[63px] transition-all duration-300 overflow-y-hidden`}
          ...
          <div className="flex flex-col gap-5">
            {props.chapter.topics.map((topic) => {
              ...
              if (props.deletedTopicsList) {
                deleted = props.deletedTopicsList.some(
                  (topicArr) =>
                    topicArr.chapterId === props.chapter.id &&
                    topicArr.topicId === topic.id,
                );
              }
              return (
                <AdventureTopic
                  key={`topic-${topic.id}`}
                  addTopicToAdventure={props.addTopicToAdventure}
                  removeFromDeletedList={props.removeFromDeletedList}
                  removeFromModifiedTopicsList={props.removeFromModifiedList}
                  onDeleted={deleted}
                  putInDeletedList={props.putInDeletedList}
                  putInModifiedTopicsList={props.putInModifiedTopicsList}
            ...
            {props.newTopic !== null &&
            props.newTopic &&
            props.chapter.id === props.newTopic.chapterId ? (
              <AdventureTopic
                cancelAddNewTopic={cancelAddNewTopic}
                addTopicToAdventure={props.addTopicToAdventure}
                editMode={true}
                chapterId={props.chapter.id}
                topic={props.newTopic}
                setTopic={props.setNewTopic}
              />
            ) : (
              <div className="w-[200px] mt-5">
                <Button onClick={startAddNewTopic}>+ Novo Tópico</Button>
              </div>
            )}
            {/* botão novo */}
            <div
              className={`w-[200px] mt-2 ${allCompleted ? "" : "pointer-events-none"}`}
            >
              <Button
                variant={"challenge"}
                className={clsx({
