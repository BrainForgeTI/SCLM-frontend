...
export const SignUpPage = () => {
      ...
      fields: [
        {
          ...
          fieldPlaceholder: "Seu nome",
          fieldValidator: (value: string) => {
            return !validator.emptyString(value);
          },
        },
        {
          fieldId: "lastName",
          fieldType: "text",
          fieldLabel: "Último nome",
          fieldPlaceholder: "Seu sobrenome",
          fieldValidator: (value: string) => {
            return !validator.emptyString(value);
          },
        },
        {
          ...
          fieldPlaceholder: "Seu nome de usuário",
          fieldValidator: (value: string) => {
      ...
      fields: [
        {
          ...
          fieldLabel: "",
          fieldPlaceholder: "",
          fieldValidator: (value: File) => {
            return validator.validateImagUpload(value);
          },
        },
        {
          fieldId: "dateOfBirth",
          ...
          fieldPlaceholder: "Data de aniversário",
          fieldValidator: (value: string) => {
        ...
        },
        {
          ...
          fieldType: "select",
          fieldLabel: "Gênero",
          fieldPlaceholder: "Gênero",
          selectValues: [
            { label: "Masculino", value: "male" },
            { label: "Feminino", value: "female" },
            { label: "Outro", value: "other" },
          ],
          fieldValidator: (value: any) => {
            return !validator.emptyString(value);
          },
        },
      ],
    },
    {
      ...
      fields: [
        ...
        },
        {
          ...
          fieldPlaceholder: "Confirme a senha",
          fieldValidator: (value: any) => {
            return equalPassword(value[0], value[1]);
          },
        },
      ],
    },
  ...
  function handleSignUpFormInput(
  ...
  function handleSignUpImageInput(
    event: ChangeEvent<HTMLInputElement>,
    field: string,
  ) {
  ...
  function handleCodeInput(
    ...
    if (event.target.value === "") {
      ...
      if (
        index < validateCodesRef.length - 1 &&
        validateCodesRef[index + 1].current
      ) {
        validateCodesRef[index + 1].current?.focus();
      }
    }
  ...
  function checkAndSetValidStep(step: number) {
    ...
    if (foundStep) {
        ...
        if (field.equalTo) {
          if (
            !field.fieldValidator([
              signUpForm[field.fieldId],
              signUpForm[field.equalTo],
            ])
          ) {
      ...
      setSteps((prev) =>
        prev.map((item) =>
          item.step === step
            ? { ...item, validStep: validFieldsAtThisStep }
            : item,
        ),
      );
    }
  ...
  useEffect(() => {
    setPasswordStatus({
      ...
      hasSpecialCharacter: validator.validateHasSpecialCharacter(
        signUpForm.password,
      ),
    ...
    >
      <div className="w-[320px] lg:w-[384px] flex lg:block justify-center">
        <div className="w-full">
          <h1 className="text-base-content text-[20px] font-medium text-center mb-5">
            {steps[currentStep.step].title}
          </h1>

          <AuthFormsBox>
            <div className="w-full flex flex-col gap-5">
              {steps[currentStep.step].showReturn ? (
                      ...
                      <>
                        {step.fieldType !== "file" ? (
                          <AuthChooseInput
                            ...
                            step={step}
                            index={index}
                            handleInput={handleSignUpFormInput}
                          />
                        ) : (
                          <AuthChooseInput
                            key={`map-input-${index}`}
                            ...
                            step={step}
                            index={index}
                            handleInput={handleSignUpImageInput}
                          />
                        )}
                      </>
                    );
                      ...
                      <ul className="w-full flex flex-col gap-3">
                        <li key="min-character" className="w-full ps-3">
                          <AuthPasswordStatus
                            ...
                            status={passwordStatus.hasMinLength}
                          />
                        </li>
                        <li key="special-character" className="w-full ps-3">
                          <AuthPasswordStatus
                            label="Pelo menos um carácter especial."
                            status={passwordStatus.hasSpecialCharacter}
                          />
                        </li>
                        <li key="has-number" className="w-full ps-3">
                          <AuthPasswordStatus
                            label="Pelo menos um número."
                            status={passwordStatus.hasNumber}
                          />
                        </li>
                        <li key="has-uppercase" className="w-full ps-3">
                          <AuthPasswordStatus
                            label="Pelo menos uma letra maiúscula."
                            status={passwordStatus.hasUppercase}
                          />
                        </li>
                        <li key="has-lowercaase" className="w-full ps-3">
                          <AuthPasswordStatus
                            label="Pelo menos uma letra minúscula."
                            status={passwordStatus.hasLowercase}
                          />
                        </li>
                      </ul>
                    </>
              ...
              {currentStep.step === 3 ? (
                <div className="text-base-content">
                  ...
                  <p className="text-base-content/70 text-[14px] text-center mt-5">
                    Enviamos um código de confirmação para o email informado
                    anteriormente. Digite o código abaixo para validar sua
                    conta. Outro código poderá ser gerado após 1 minuto.
                  </p>
                    ...
                    {steps[3].fields.map((step, index) => {
                      return (
                        <AuthCodeInput
                          form={signUpForm}
                          id={step.fieldId}
                          ref={validateCodesRef[index]}
                          step={step}
                          value={signUpForm[step.fieldId]}
                          index={index}
                          handleInput={handleCodeInput}
                          key={`code-${step.fieldId}`}
                        />
                      );
                    })}
                  </div>
              ...
              <div className="w-full mt-3">
                ...
                <AuthSubmitButton
                  action={goToNextStep}
                  label={steps[currentStep.step].buttonLabel}
                  styles={
                    steps[currentStep.step].validStep
                      ? "bg-primary text-primary-content border-transparent mt-6 text-[18px]"
                      : "mt-6 bg-primary/0 border-primary-content/50 text-primary-content/50 text-[18px]"
                  }
                  disabled={steps[currentStep.step].validStep}
                />

                {currentStep.step === 3 ? (
                  <AuthSubmitButton
                    action={sendOtherEmailCode}
                    label={`Enviar outro código (${timeToNextEmailCode}s)`}
                    disabled={timeToNextEmailCode === 0 ? true : false}
                    styles={`${timeToNextEmailCode === 0 ? "bg-primary/50 text-primary-content border-transparent mt-6" : "mt-6 bg-primary/0 border-primary-content/50 text-primary-content/50"} text-[12px]`}
                  />
                ) : (
                  <></>
                )}
